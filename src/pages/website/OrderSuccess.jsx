...
const OrderSuccess = () => {
    ...
    useEffect(() => {
        ...
        const loadOrder = async () => {
            try {
                ...
                // CONDITION 1: ORDER ID REQUIRED
                if (!orderId) {
                    setError("Order ID is missing.");
                    return;
                }

                // CONDITION 2: USER REQUIRED
                if (!user) {
                    setError("Please login to view this order.");
                    return;
                }

                // CONDITION 3: API EXISTENCE
                ...
                // CONDITION 5: API FAILURE
                if (response.success === false) {
                    setError(response.message || "We could not find this order.");
                    return;
                }
                ...
                // CONDITION 6: ORDER NOT FOUND
                if (!foundOrder || typeof foundOrder !== "object") {
                    setError("We could not find this order. It may have been removed or the order ID may be incorrect.");
                    return;
                }

                // CONDITION 7: USER SECURITY CHECK
                const orderUserId =
                    foundOrder?.user_id ??
                    foundOrder?.userId ??
                    foundOrder?.user?._id ??
                ...
                if (orderUserId && loggedInUserId && String(orderUserId) !== String(loggedInUserId)) {
                    setError("You are not authorized to view this order.");
                    return;
                }


                // CONDITION 8: SET ORDER
                if (isMounted) {
                    setOrder(foundOrder);
                }
                ...
                if (isMounted) {
                    setError(err?.message || "Unable to load order details. Please try again.");
                }
        ...
        return (
            <div className="min-h-[70vh] bg-gray-50 flex items-center justify-center px-4 py-10">
                <div className="w-full max-w-lg bg-white rounded-2xl shadow-lg p-8 text-center">
                    ...
                    <p className="mt-3 text-gray-500 leading-6">{error || "We could not find this order."}</p>
                    {orderId && (
                        <p className="mt-4 text-sm text-gray-400 break-all">Order ID: {orderId}</p>
                    )}
    ...
    // TOTAL
    const total = Number(order.totalAmount ?? order.total ?? order.grandTotal) || subtotal + shipping + tax;

    // ORDER ID DISPLAY
    const displayOrderId = order?._id ?? order?.id ?? order?.orderId ?? orderId;
                        ...
                        <div className="divide-y">
                            {items.length > 0 ? (
                                items.map((item, index) => {
                                    ...
                                    const title = item.title || item.name || book?.title || "Book";
                                    const image = item.images || book?.images || book?.images || "";
                                    const author = item.author || book?.author || "";
                                    return (
                                        <div key={item._id || item.id || item.bookId || book?._id || index} className="p-5 flex gap-4">
                            ...
                            {typeof shippingAddress === "string" ? (
                                ...
                                <>
                                    ...
                                    {(shippingAddress.pincode || shippingAddress.postalCode) && (
                                        <p>PIN:{" "} {shippingAddress.pincode || shippingAddress.postalCode}</p>
                                    )}
